...
contract('CannCoinCash', function (accounts) {
  ...
  it('transfers from a delegated account', function () {
    return CannCoinCash.deployed().then(function (instance) {
      tokenInstance = instance;
      tokenInstance.transferFrom(accounts[0], accounts[1], 9999999999);
    }).then(assert.fail).catch(function (error) {
      assert(error.message.indexOf('revert') >= 0, 'it throws and error if from account does not have enought');
    });
  });
